...
// sort shoes by price (descending or ascending)
app.get('/sort', (req, res) => {
    let sortBy = req.query.by === 'price' ? 'price' : 'price';
    let order = req.query.order === 'asc' ? 'ASC' : 'DESC';
    const sql = `SELECT * FROM Product ORDER BY ${sortBy} ${order}`;
    db.query(sql, (err, results) => {
        if (err) {
            console.error(err);
            return res.status(500).send('Error fetching sorted shoes');
        }
        res.render('sort', {
            items: results,
            sortBy,
